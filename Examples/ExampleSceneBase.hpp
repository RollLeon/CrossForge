...
namespace CForge {
    ...
    protected:
        ...
        virtual void listen(GLWindowMsg Msg) override {
            ...
            m_Cam.projectionMatrix(Msg.iParam[0] / m_RenderBufferScale, Msg.iParam[1] / m_RenderBufferScale,
                                   CForgeMath::degToRad(45.0f), 0.1f, 1000.0f);
        ...
        virtual void
        initWindowAndRenderDevice(bool UseGBuffer = true, uint32_t DirLightCount = 1, uint32_t PointLightCount = 1,
        ...
        virtual void initCameraAndLights(bool CastShadows = true) {
            ...
            m_BGLight.init(BGLightPos, -BGLightPos.normalized(), Vector3f(1.0f, 1.0f, 1.0f), 1.5f,
        ...
        void defaultKeyboardUpdate(Keyboard *pKeyboard) {
            ...
            if (m_RenderWin.keyboard()->keyPressed(Keyboard::KEY_C, true)) {
                auto mode = glfwGetInputMode(glfwGetCurrentContext(), GLFW_CURSOR);
                glfwSetInputMode(glfwGetCurrentContext(),GLFW_CURSOR, mode==GLFW_CURSOR_HIDDEN?GLFW_CURSOR_NORMAL:GLFW_CURSOR_HIDDEN);
            }
        ...
        void initFPSLabel(void) {
            ...
            m_FPSLabel.position(m_RenderWin.width() - pFont->computeStringWidth("FPS: 60.0") - 10,
                                m_RenderWin.height() - 30);
